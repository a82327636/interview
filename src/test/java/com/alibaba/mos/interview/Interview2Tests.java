...
import com.alibaba.mos.util.structure.BigCapHeap;
...
class Interview2Tests {
    ...
    void readDataFromExcelWithHandlerTest() {
        ...
        skuReadService.loadSkus((SkuDO skuDO) -> {
    ...
    void statisticsDataTest() {
        ...
        // todo 因为公司要开会没有时间写第一点，思路是可以通过从1元到1万元做一个桶排序，然后根据二分法取值，直到取到中间值。

        // 业务处理
        if(skuDOS!=null && skuDOS.size()>0){
            BigCapHeap miaoBigCapHeap = new BigCapHeap(5); // miao库容量大顶堆
            BigCapHeap tmallbigCapHeap = new BigCapHeap(5); // tmall库容量大顶堆
            BigCapHeap intimebigCapHeap = new BigCapHeap(5); // intime库容量大顶堆
            Double totalValue = new Double(0); // 总价值
            for(SkuDO skuDO:skuDOS){
                if(skuDO.getInventoryList()!=null){
                    for(ChannelInventoryDO channelInventoryDO : skuDO.getInventoryList()){
                        BigCapHeap.CData cData = new BigCapHeap.CData();
                        cData.setId(skuDO.getId());
                        cData.setInventory(channelInventoryDO.getInventory().doubleValue());
                        if("MIAO".equals(channelInventoryDO.getChannelCode())){
                            miaoBigCapHeap.insert(cData);
                        }else if("TMALL".equals(channelInventoryDO.getChannelCode())){
                            tmallbigCapHeap.insert(cData);
                        }else if("INTIME".equals(channelInventoryDO.getChannelCode())){
                            intimebigCapHeap.insert(cData);
                        }
                        totalValue = totalValue + skuDO.getPrice().doubleValue() * channelInventoryDO.getInventory().doubleValue();
                    }
                }
            }
            log.info("MIAO库容量:={}", miaoBigCapHeap.getA());
            log.info("TMALL库容量:={}", tmallbigCapHeap.getA());
            log.info("INTIME库容量:={}", intimebigCapHeap.getA());

            log.info("totalValue总价值:={}", totalValue);
        }
